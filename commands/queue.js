...
module.exports = {
  name: "kuyruk",
  description: "Çalma listesini görüntülersiniz.",
  ...
  run: async (client, interaction) => {
    ...
    if (!queue.tracks[0]) return interaction.reply({ content: `Bu şarkıdan sonra şarkı yok. ❌`, ephemeral: true }).catch(e => { })
    if (cmds) return interaction.reply({ content: `Komut zaten bu kanalda kullanılıyor. ❌\nhttps://discord.com/channels/${interaction.guild.id}/${interaction.channel.id}/${cmds}`, ephemeral: true }).catch(e => { })
    ...
    const generateEmbed = async (start) => {
      ...
      return new EmbedBuilder()
        ...
        .setColor('007fff')
        .setDescription(`Şu anda çalınan şarkı: \`${queue.current.title}\`
    ...
    await interaction.reply({
      ...
      collector.on("end", async (button) => {
        ...
        const embed = new EmbedBuilder()
          .setTitle(`Sunucu Müzik Listesi - Süre Doldu!`)
          .setThumbnail(interaction.guild.iconURL({ size: 2048, dynamic: true }))
          .setColor('007fff')
