...
fs.readdir("./events", (_err, files) => {
  files.forEach((file) => {
    ...
    console.log(`Şu event yüklendi: ${eventName}`);
...
fs.readdir(config.commandsDir, (err, files) => {
  ...
  files.forEach(async (f) => {
    try {
      ...
      console.log(`Şu komut yüklendi: ${props.name}`);
    } catch (err) {
      console.log(err);
    }
  });
...
player.on('trackAdd', (queue, track) => {
  if (queue) {
    if (queue.metadata) {
      queue.metadata.send({ content: `**${track.title}** adlı şarkı oynatma listesine eklendi. ✅` }).catch(e => { })
    }
  }
});
...
player.on('channelEmpty', (queue) => {
  if (queue) {
    if (queue.metadata) {
      queue.metadata.send({ content: `Ses kanalında kimse olmadığı için ayrıldım. ❌` }).catch(e => { })
    }
  }
});
...
player.on('queueEnd', (queue) => {
  if (client.config.opt.voiceConfig.leaveOnTimer.status === true) {
    ...
    if (queue.metadata) {
      queue.metadata.send({ content: `Tüm çalma sırası bitti, biraz daha müzik dinleyebilirsin. ✅` }).catch(e => { })
    }
  }
});
